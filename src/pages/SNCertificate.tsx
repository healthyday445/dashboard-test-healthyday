...
export default function SNCertificate() {
  ...
  const previewOverride = searchParams.get("preview_sn") !== null || searchParams.get("preview") !== null || searchParams.get("preview_levels") !== null;
  ...
  const [loadingStudent, setLoadingStudent] = useState(!!mobile);
  ...
  // Check local storage & server status
  useLayoutEffect(() => {
    if (!mobile) return;
    ...
    >
      ...
      <div className="w-full max-w-3xl px-4 mt-8 text-center">
        ...
        <p className="text-[#5A6577] text-base md:text-lg mt-2 max-w-xl mx-auto">
          Celebrate your endurance and accomplishment! Verify your name and generate your official completion certificate.
        </p>
      </div>
