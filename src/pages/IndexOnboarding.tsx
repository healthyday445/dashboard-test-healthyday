...
import introThumbTel from "@/assets/intro_tel.webp";
import introThumbEng from "@/assets/intro_eng.webp";

/**
 * Onboarding/"day 0" screen — a student who has registered but whose free batch hasn't
 * started yet (or has forceDay=0 QA preview active). Extracted out of IndexFourteenDaysV2.tsx
 * into its own lazy-loaded module so this (video thumbnails, intro-session assets) isn't part
 * of the bundle for the much more common "batch already active" path.
 *
 * Contest-enrolled students (`last_referral_contest_id` present on the student record) see
 * the Refer & Win contest teaser in place of the generic ReferWinCard — see
 * docs.local/leaderboard_contest_plan.md and the contest-enrolled-free-batches memory note.
 */
...
interface IndexOnboardingProps {
  mobile?: string;
  studentData: any;
  studentStatus?: string;
  isForceOnboardingPreview: boolean;
  globalTimeParam: string | null;
}

const IndexOnboarding: React.FC<IndexOnboardingProps> = ({
  mobile,
  studentData,
  studentStatus,
  isForceOnboardingPreview,
  globalTimeParam,
}) => {
  ...
  // see the Refer & Win contest teaser in place of the generic ReferWinCard.
  ...
  return (
    <div className="hd-page bg-white" style={{ fontFamily: "Outfit, sans-serif" }}>
      ...
      {(() => {
        ...
        const _totalMin = (() => {
          if (globalTimeParam) {
            const isPM = globalTimeParam.toLowerCase().endsWith("pm");
            const s = globalTimeParam.toLowerCase().replace("am", "").replace("pm", "");
