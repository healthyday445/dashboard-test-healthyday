...
const Index = () => {
  ...
  if (hasBatchAccess) {
    ...
    // Build 14 status values: "green" | "grey" | "orange" | "red"
    const dayStatus = Array.from({ length: 14 }, (_, i) => {
      const dayNum = i + 1;
      if (dayNum > currentDay) return "grey";
      const raw = attendance[i];
      if (raw === "absent") return "red";
      if (raw === "partial") return "orange";
    ...
    const DayBox = ({ status, dayLabel }: { status: string; dayLabel: string }) => (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "42px" }}>
        <div style={{
          ...
          background: status === "grey" ? "#E0E0E0" : status === "red" ? "#FF3B30" : status === "orange" ? "#FEAB27" : "#0D9400",
    ...
    return (
      <div className="mx-auto w-[412px] min-h-screen bg-white" style={{ fontFamily: "Outfit, sans-serif" }}>
        ...
        <div style={{ padding: "28px 20px 0" }}>
          {/* Header row */}
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px" }}>
            <h3 style={{ color: "#202020", fontFamily: "Outfit", fontSize: "18px", fontWeight: 700, margin: 0 }}>
              Referral Milestones
            </h3>
            <span
              onClick={() => navigate(`/referral?count=${studentData?.total_referral_count ?? 0}&mobile=${mobile || ""}`)}
