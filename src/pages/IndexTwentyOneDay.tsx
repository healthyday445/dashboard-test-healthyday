...
import thumbLiveQA from "@/assets/bonus/liveqa.webp";
...
const IndexTwentyOneDay = ({ initialStudentData, onSwitchToJourney }: IndexTwentyOneDayProps = {}) => {
  ...
  // Memoized so its identity is stable across renders when the QA param hasn't actually
  ...
  const effectiveInitialData = useMemo(() => {
    ...
    return previewStudentData ?? initialStudentData;
  }, [previewDashboardKey, initialStudentData]);
  ...
  if (hasBatchAccess) {
    ...
    const BONUS_DAYS = lang === "English" ? [5, 8, 12, 15, 19] : [4, 8, 11, 15, 18];
      ...
      const getBonusInfo = (day: number, l: string): BonusInfo => {
        if (l === "Telugu") {
          switch (day) {
            ...
            case 18: return { name: "Live Q&A", fullName: "Live Q&A Session at 8:30 PM", startMin: 20 * 60 + 30, videoId: "SyjnCjDtNS8", sessionLink: "https://start.dailyyogawithjagan.com/liveqa", thumbnail: thumbLiveQA, liveDuration: 60, activeEndOffset: 60 };
          ...
          switch (day) {
            ...
            case 19: return { name: "Live Q&A", fullName: "Live Q&A Session at 8:30 PM", startMin: 20 * 60 + 30, videoId: "SyjnCjDtNS8", sessionLink: "https://start.dailyyogawithjagan.com/liveqa_eng", thumbnail: thumbLiveQA, liveDuration: 60, activeEndOffset: 60 };
            default: return {} as BonusInfo;
          }
        }
      };
  ...
  // All paid students render the same shared IndexPaid dashboard, regardless of which
  // free-batch cohort they originally joined (this used to have its own separate inline
  // implementation, which is how plan-type and feature bugs like the missing "Update
  // Attendance"/"Referral Rewards" UI diverged from the 14-day-v2 cohort's paid view).
  if (isPaid) {
    return (
      <IndexPaid
        ...
        verifiedReferralCount={studentData?.total_referral_count ?? 0}
      />
    );
  }
